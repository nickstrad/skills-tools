...
func (c Coach) load(id string) (Project, error) {
	...
	for i, l := range p.Lessons {
		...
		for _, dep := range l.Prerequisites {
			if !seen[dep] {
				return p, fmt.Errorf("%s: prerequisite %s must occur earlier", l.Slug, dep)
			}
		}
		seen[l.Slug] = true
		if l.Available {
...
func (c Coach) done(topic string, l Lesson) (bool, error) {
	_, err := os.Stat(c.receipt(topic, l))
	if errors.Is(err, os.ErrNotExist) {
		return false, nil
	}
	return err == nil, err
...
func (c Coach) selectTopic(id string) error {
	...
	f, err := os.CreateTemp(c.state, ".active-")
	if err != nil {
		return err
	}
	defer os.Remove(f.Name())
	...
	if err := os.Rename(f.Name(), filepath.Join(c.state, "active")); err != nil {
		return err
	}
	fmt.Fprintln(c.out, "Selected", id)
	return nil
}
...
func (c Coach) run(args []string) error {
	...
	if len(args) > 0 {
		...
		case "use", "check":
			...
			if args[0] == "use" {
				return c.selectTopic(args[1])
			}
			p, err := c.load(args[1])
			if err != nil {
				return err
			}
			fmt.Fprintf(c.out, "%s: valid %s agenda, %d lessons (structure only; see validation records for experiment evidence)\n", p.ID, p.Status, len(p.Lessons))
			return nil
		}
	}
	var topic string
	...
	if topic == "" {
		b, err := os.ReadFile(filepath.Join(c.state, "active"))
		if errors.Is(err, os.ErrNotExist) {
			return errors.New("no selected project; use systemscoach TOPIC route or systemscoach use TOPIC")
		}
		if err != nil {
			return err
		}
	...
	if len(args) == 1 && args[0] == "route" {
		...
		for i, l := range p.Lessons {
			...
			fmt.Fprintf(c.out, "%d. %s — %d min · %s\n   %s\n", i+1, l.Title, l.Minutes, status, l.Outcome)
			total += l.Minutes
		}
	...
	if len(args) > 0 {
		if value, err := strconv.Atoi(args[0]); err == nil {
			...
			args = args[1:]
			if n < 1 {
				return errors.New("lesson number must be positive")
			}
		}
	}
	action := "lesson"
	...
	if p.Status != "approved" {
		return errors.New("agenda is a draft; agree the learning agenda before authoring or taking lessons")
	}
	if n == 0 {
		for i, l := range p.Lessons {
			done, err := c.done(topic, l)
	...
	if n > len(p.Lessons) {
		return fmt.Errorf("lesson %d is outside this %d-lesson route", n, len(p.Lessons))
	}
	l := p.Lessons[n-1]
	if !l.Available {
		return fmt.Errorf("lesson %d (%s) is planned, not authored; ask for the next small batch", n, l.Title)
	}
	if action == "done" {
		b, err := json.Marshal(struct {
			...
			Revision    int
			CompletedAt string
		}{topic, l.Slug, l.Revision, time.Now().UTC().Format(time.RFC3339Nano)})
		if err != nil {
			return err
		}
...
func main() {
	...
	state := os.Getenv("SYSTEMSCOACH_STATE")
	if state == "" {
		state = filepath.Join(root, ".state")
	}
	if err := (Coach{root, state, os.Stdout}).run(os.Args[1:]); err != nil {
		fmt.Fprintln(os.Stderr, "systemscoach:", err)
		os.Exit(1)
	}
}
