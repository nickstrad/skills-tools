...
1. `every-change-is-a-wal-record` (writes-data, pg_walinspect). Capture `pg_current_wal_lsn()`
   before/after an insert/update/delete; `pg_get_wal_records_info(start, end)` shows Heap INSERT /
   HOT_UPDATE / DELETE records with their lengths; commit record (Transaction COMMIT). Expect record
   types. Lens: the log is the source of truth; replication and recovery both consume it.
2. `full-page-writes-after-checkpoint` (writes-data). `checkpoint;` then update one row: the WAL
   record is ~8 KB (FPI); update again: ~100 bytes. Show `fpi_len` in pg_walinspect. Lens: torn page
   protection; why checkpoints cause WAL bursts.
3. `commit-means-fsync` (privileged). `synchronous_commit` on vs off: time 2000 single-row commits
   in a `do` block? A do block is one txn. Instead use `\timing` and a psql script with
   `\gexec`-generated 500 inserts autocommit... simpler: `pgbench -i -s 1` then `pgbench -T 5 -c 1`
   with sync on vs off (shell lesson, run as postgres). Expect tps differs (report real numbers,
   note it depends on disk). Also `pg_test_fsync -s 2`. Lens: durability latency floor = fsync;
   group commit; the "lose last N ms" trade.
4. `wal-files-and-recycling` (shell + tool). `pg_switch_wal()`, `\! ls $PGLAB/primary/pg_wal`,
   `pg_walfile_name`, archive dir grows (archive_mode on). Lens: segments, the archive as the
   off-box durability boundary.
5. `crash-and-redo` (dangerous, mixed, 2 sessions). Insert rows, note LSN,
   `pg_ctl -m immediate
   stop` (as postgres via `\! su`? No: run the shell part as postgres in a
   shell lesson). Restart; log shows "database system was interrupted", "redo starts at X", "redo
   done at Y"; the committed rows are present, an uncommitted one is not. Lens: ARIES-style redo;
   commit = WAL durable, not data file durable.
6. `wal-replay-is-deterministic` (read-only). Use `pg_waldump` on the segment covering the crash to
   read the same records the server replayed; correlate with the log's redo LSNs. Lens: the log is a
   replayable, inspectable stream; same mechanism drives standbys.
7. `wal-size-of-operations` (writes-data). Compare WAL bytes generated by: 10k single-row inserts in
   one txn vs 10k autocommit inserts; an `update` of an indexed vs unindexed column; `create index`.
   Use `pg_wal_lsn_diff`. Lens: write amplification and batching.
