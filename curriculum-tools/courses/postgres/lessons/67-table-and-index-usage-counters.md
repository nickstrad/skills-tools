# Decide whether a zero-scan index still earns its place
...
## Overview
A zero scan count describes an observation window, not an index's full responsibility. Compare query plans and scoped counter deltas, then test a uniqueness constraint whose backing index received no query scans. Add a missing workload before proposing an index removal.
...
## Setup
```sql
drop table if exists obs_usage_orders;
