# Attribute I/O counters without inventing disk latency
...
## Overview
An I/O counter is useful only if you know its scope and interval. Compare relation growth during loading with two subsequent scans, join PostgreSQL 16's backend/object/context counters to the actual query plan, and decide which conclusions need operating-system evidence.
...
insert into obs_io_load select g, repeat('x',700) from generate_series(1,100000) g;
select pg_stat_force_next_flush();
select pg_stat_clear_snapshot();
...
select count(*) as rows, sum(length(pad)) as payload_bytes,
       count(*) = 100000 and sum(length(pad)) = 70000000 as answer_ok
from obs_io_load;
commit;
select pg_stat_force_next_flush();
select pg_stat_clear_snapshot();
...
## Expected result
The core returns rows=100000, payload_bytes=70000000 and answer_ok=true. In the 128MB lab its heap is larger than one quarter of shared_buffers. The variation returns rows=10000 and payload_bytes=7000000, also answer_ok=true, and its heap is below that boundary.
