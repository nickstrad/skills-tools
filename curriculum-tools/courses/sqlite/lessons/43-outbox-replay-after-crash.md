...
## Optional variation
For the deliberate failure comparison on fresh disposable sender/receiver files, commit the first debit before inserting its receipt and retain the sender-kill boundary before that later receipt transaction. At the boundary the receiver has balance 90 and no receipt. Replaying the original batch through deliver therefore debits again and commits balance 80 with one receipt. The core's balance-90 replay assertions should reject this broken protocol; do not interpret their failure as successful deduplication. An HTTP effect accepted before its local receipt creates the same uncertainty, and SQLite cannot roll back the remote acceptance.
