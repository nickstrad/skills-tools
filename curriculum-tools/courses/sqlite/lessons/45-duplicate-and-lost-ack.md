...
## Optional variation
On fresh disposable files, repeat with the identity_guard payload-comparison INSERT omitted from deliver. The changed-amount request then returns successfully with zero new receipts: the receiver retains the original debit and balance 85, but the sender could mistake that success for acceptance of amount 99. The existing unexpected-acceptance branch must reject this broken variant. Keep payload validation in the working protocol. A receipt extended with the original accepted response can return that same response on valid replay; it must not fabricate a response for conflicting intent.
