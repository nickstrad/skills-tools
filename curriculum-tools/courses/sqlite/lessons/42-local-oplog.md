...
## Optional variation
Repeat Setup and Run, moving only the second `UPDATE device SET next_seq=next_seq+1;` to immediately after its ROLLBACK. The committed body and clock remain offline edit and 1, with one durable operation, but next_seq is now 3. A subsequent successful operation would use sequence 3 without a committed sequence 2, leaving a receiver waiting for that missing identity. A restored device needs reconciled retained history and allocation/clock metadata before reusing device-a/g1; otherwise it must establish a new generation rather than assume an old counter is safe.
