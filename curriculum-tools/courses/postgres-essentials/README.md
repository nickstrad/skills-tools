...
fifteen are available**; the remaining 25 are planned and will be authored in small batches.
...
retries, request reconciliation and transaction lifetime. No TOAST/cache/XID tour or new cluster
installation is needed before starting them.
...
reflection follows the experiment; a brief feedback chat after lesson 15 guides the next batch.
...
before setup. Lesson 4 uses one experiment terminal; the other SQL lessons use two. Lessons 10–11
use one shell with supplied Python 3/psql clients that create unique pe_retry_* or pe_unknown_*
schemas, open their own connections and remove the schemas on exit. To stop early, ROLLBACK in each
SQL session, restore settings as shown and drop the exact lesson table. For shell clients, Ctrl-C
runs cleanup; check the schema-removal record. Lessons 3–4 leave the shared pgstattuple extension
installed; table-specific autovacuum settings disappear with their tables.
