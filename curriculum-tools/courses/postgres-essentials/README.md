...
ten are available**; the remaining 30 are planned and will be authored in small batches.
...
retries. No TOAST/cache/XID tour or new cluster installation is needed before starting them.
...
reflection follows the experiment; a brief feedback chat after lesson 10 guides the next batch.
...
database lab. Lessons 1–9 prepare only their own `pe_*` table and drop it at the end. Finish any
earlier transactions before setup. Lesson 4 uses one experiment terminal; the other SQL lessons use
two. Lesson 10 uses one shell and a supplied Python 3/psql client that creates a unique `pe_retry_*`
schema, opens its own connections, and removes the schema on exit. To stop early, ROLLBACK in each
open experiment session, then drop the exact table named by that lesson's final command. For lesson
10, Ctrl-C runs the client's cleanup; check its schema-removal record. Lessons 3–4 leave the shared
pgstattuple extension installed; their table-specific autovacuum settings disappear with their
tables.
